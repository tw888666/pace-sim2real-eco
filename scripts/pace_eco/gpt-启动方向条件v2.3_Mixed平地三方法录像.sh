#!/usr/bin/env bash
# 用法：GPU编号 输出根目录；固定录制seed1、holdout batch0/env2平地中等难度实例。
...
for method in task_only fixed_weight eco; do
    ...
    log="${output_root}/gpt_GPU${gpu}_flat_${method}_seed1录像.log"
    command=("${PACE_V23_CONDA}" run -n "${PACE_V23_ENV}" --no-capture-output "${PACE_V23_ISAACLAB_ROOT}/isaaclab.sh" -p "${PACE_V23_ROOT}/scripts/pace_eco/gpt_录制方向条件v2.3_Mixed三方法.py" --task "${task}" --checkpoint "${checkpoint}" --ppo_seed 1 --batch_index 0 --env_index 2 --energy_reference_json "${PACE_V23_BUDGET}" --holdout_authorization "${authorization}" --duration_s 20 --video_fps 25 --resolution 1280 720 --output_dir "${method_output}" --headless --device cuda:0)
...
[[ "${video_count}" -eq 3 ]] || pace_v23_fail "三方法录像数量错误：${video_count}"
printf '[PACE-v2.3] GPU%s平地三方法录像全部完成：%s\n' "${gpu}" "${output_root}"
